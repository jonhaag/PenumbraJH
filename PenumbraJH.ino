...
#include <HardwareSerial.h>
HardwareSerial SerialPort1(1); //use UART1
HCRVocalizer HCR(&SerialPort1,9600);
...
public:
    ...
    virtual void notify() override
    {
        ...
        else
        {
            ...
            if (!state.button.l1 && !state.button.l2 && !state.button.ps) //un-modified button presses
            {
              if (event.button_down.circle)
              {
                DEBUG_PRINTLN ("DRIVE CIRCLE PRESSED");
              }
              if (event.button_down.cross)
              {
                DEBUG_PRINTLN ("DRIVE CROSS PRESSED");

                HCR.Overload(); //Overload or electrocution sound
                DEBUG_PRINTLN ("HCR Overload Emote");
              }
              if (event.button_down.up)
              {
                DEBUG_PRINTLN ("DRIVE UP PRESSED");

                HCR.Stimulate(HAPPY,EMOTE_MODERATE); //Make a moderate happy emotion sound
                DEBUG_PRINTLN ("HCR Moderate Happy Emote");
              }
              if (event.button_down.right)
              {
                DEBUG_PRINTLN ("DRIVE RIGHT PRESSED");

                HCR.Stimulate(SAD,EMOTE_MODERATE); //Make a moderate sad emotion sound
                DEBUG_PRINTLN ("HCR Moderate Sad Emote");
              }
              if (event.button_down.down)
              {
                DEBUG_PRINTLN ("DRIVE DOWN PRESSED");

                HCR.Stimulate(MAD,EMOTE_MODERATE); //Make a moderate mad emotion sound
                DEBUG_PRINTLN ("HCR Moderate Mad Emote");
              }
              if (event.button_down.left)
              {
                DEBUG_PRINTLN ("DRIVE LEFT PRESSED");

                HCR.Stimulate(SCARED,EMOTE_MODERATE); //Make a moderate scared emotion sound
                DEBUG_PRINTLN ("HCR Moderate Scared Emote");
              }
            }

            if (state.button.l1 && !state.button.l2 && !state.button.ps) //L1 modified button presses
            {
              if (event.button_down.circle)
              {
                DEBUG_PRINTLN ("DRIVE L1 + CIRCLE PRESSED");
              }
              if (event.button_down.cross)
              {
                DEBUG_PRINTLN ("DRIVE L1 + CROSS PRESSED");
              }
              if (event.button_down.up)
              {
                DEBUG_PRINTLN ("DRIVE L1 + UP PRESSED");
              }
              if (event.button_down.right)
              {
                DEBUG_PRINTLN ("DRIVE L1 + RIGHT PRESSED");
              }
              if (event.button_down.down)
              {
                DEBUG_PRINTLN ("DRIVE L1 + DOWN PRESSED");
              }
              if (event.button_down.left)
              {
                DEBUG_PRINTLN ("DRIVE L1 + LEFT PRESSED");
              }
            }

            if (!state.button.l1 && state.button.l2 && !state.button.ps) //L2 modified button presses
            {
              if (event.button_down.circle)
              {
                DEBUG_PRINTLN ("DRIVE L2 + CIRCLE PRESSED");
                
                HCR.SetMuse(true); //Enable HCR random musing
                DEBUG_PRINTLN ("HCR Muse Enabled");
              }
              if (event.button_down.cross)
              {
                DEBUG_PRINTLN ("DRIVE L2 + CROSS PRESSED");

                HCR.SetMuse(false); //Disable HCR random musing
                DEBUG_PRINTLN ("HCR Muse Disabled");
              }
              if (event.button_down.up)
              {
                DEBUG_PRINTLN ("DRIVE L2 + UP PRESSED");

                HCR.Stimulate(HAPPY,EMOTE_STRONG); //Make a strong happy emotion sound
                DEBUG_PRINTLN ("HCR Strong Happy Emote");
              }
              if (event.button_down.right)
              {
                DEBUG_PRINTLN ("DRIVE L2 + RIGHT PRESSED");
                HCR.Stimulate(SAD,EMOTE_STRONG); //Make a strong sad emotion sound
                DEBUG_PRINTLN ("HCR Sad Scared Emote");
              }
              if (event.button_down.down)
              {
                DEBUG_PRINTLN ("DRIVE L2 + DOWN PRESSED");

                HCR.Stimulate(MAD,EMOTE_STRONG); //Make a strong mad emotion sound
                DEBUG_PRINTLN ("HCR Strong Mad Emote");
              }
              if (event.button_down.left)
              {
                DEBUG_PRINTLN ("DRIVE L2 + LEFT PRESSED");

                HCR.Stimulate(SCARED,EMOTE_STRONG); //Make a strong scared emotion sound
                DEBUG_PRINTLN ("HCR Strong Scared Emote");
              }
            }

            if (!state.button.l1 && !state.button.l2 && state.button.ps) //PS modified button presses
            {
              if (event.button_down.circle)
              {
                DEBUG_PRINTLN ("DRIVE PS + CIRCLE PRESSED");
              }
              if (event.button_down.cross)
              {
                DEBUG_PRINTLN ("DRIVE PS + CROSS PRESSED");
              }
              if (event.button_down.up)
              {
                DEBUG_PRINTLN ("DRIVE PS + UP PRESSED");
              }
              if (event.button_down.right)
              {
                DEBUG_PRINTLN ("DRIVE PS + RIGHT PRESSED");
              }
              if (event.button_down.down)
              {
                DEBUG_PRINTLN ("DRIVE PS + DOWN PRESSED");
              }
              if (event.button_down.left)
              {
                DEBUG_PRINTLN ("DRIVE PS + LEFT PRESSED");
              }
            }

            if (event.button_down.l3) // L3 only - since drive stick, should not be used in combination with anything else
            {
              DEBUG_PRINTLN ("DRIVE L3 PRESSED");
            }
...
//DFPlayer Startup
//SerialPort1.begin(9600, SERIAL_8N1, 2, 4);
//myDFPlayer.begin(SerialPort1);
//myDFPlayer.volume(10);

//myDFPlayer.play(1);

//HCR Startup
SerialPort1.begin(9600, SERIAL_8N1, 35, 14);
HCR.begin();

HCR.SetMuse(false);

HCR.Muse(5,50); //Set min, max time between musings in seconds
